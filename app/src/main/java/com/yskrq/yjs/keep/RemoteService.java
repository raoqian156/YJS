...
public class RemoteService extends Service {
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    bindService(new Intent(RemoteService.this, LocalService.class), connection, Context.BIND_ABOVE_CLIENT);
  ...
  private ServiceConnection connection = new ServiceConnection() {
    ...
    public void onServiceDisconnected(ComponentName name) {
      if (KeepAliveService.stop) {
        Log.e("RemoteService", "onServiceDisconnected.34:");
        return;
      }
      Intent intent = new Intent(RemoteService.this, KeepAliveService.class);
      //将KeepAliveService和RemoteService进行绑定
      RemoteService.this.bindService(intent, connection, Context.BIND_ABOVE_CLIENT);
    }
